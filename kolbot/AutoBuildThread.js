...
function spendStatPoints () {
	...
	if (me.charlvl >= Config.AutoBuild.RespecLevel) {
			stats = AutoBuildRecpecTemplate[me.charlvl].StatPoints;
		}
	else {
			stats = Config.AutoBuild.Template[me.charlvl].StatPoints;
	}
...
function spendStatPointsRespec () {
	var stats;
	var errorMessage = "\nInvalid stat point set in build template "+getTemplateFilename()+" at level "+me.charlvl;
	var spentEveryPoint = true;
	var unusedStatPoints = me.getStat(4);
	...
	for (levels <= me.charlvl) {
		stats = AutoBuildRecpecTemplate[levels].StatPoints;
...
function getTemplateFilename () {
	...
	if (me.charlvl >= Config.AutoBuild.RespecLevel) {
			buildType = Config.AutoBuild.RespecTemplate;
	}
	else {
			buildType = Config.AutoBuild.Template;
	}
...
function spendSkillPoints () {
	...
	if (me.charlvl >= Config.AutoBuild.RespecLevel) {
			skills = AutoBuildRecpecTemplate[me.charlvl].SkillPoints;
		}
	else {
			skills = Config.AutoBuild.Template[me.charlvl].SkillPoints;
	}
...
function spendSkillPointsRespec () {
	var skills;
	var errInvalidSkill = "\nInvalid skill point set in build template "+getTemplateFilename()+" for level "+me.charlvl;
	var spentEveryPoint = true;
	var unusedSkillPoints = me.getStat(5);
	...
	for (levels < me.charlvl) {
			skills = AutoBuildRecpecTemplate[levels].SkillPoints;
...
function main () {
	try {
		...
		while (true) {
			...
			if (me.charlvl >= Config.AutoBuild.RespecLevel && canSpendPoints() > 20 {
				AutoBuild.print("Respec Detected!");
				spendSkillPointsRespec();
				spendStatPointsRespec();	
			}
