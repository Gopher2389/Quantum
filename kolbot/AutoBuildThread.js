...
function spendStatPointsRespec () {
	...
	var unusedStatPoints;
	var len;
	
	for (var levels = 2; levels < me.charlvl+1; levels++) {
		stats = AutoBuildTemplate[levels].StatPoints;
		len = stats.length;
		unusedStatPoints = me.getStat(4);
...
function spendSkillPointsRespec () {
	...
	for (var levels = 2; levels < me.charlvl+1; levels++) {
		skills = AutoBuildTemplate[levels].SkillPoints;
		len = skills.length;
		unusedSkillPoints = me.getStat(5);
...
function main () {
	try {
		...
		while (true) {
			...
			if (me.charlvl >= Config.AutoBuild.RespecLevel && me.getStat(5) > me.charlvl*0.75) {
