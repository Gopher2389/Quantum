/** This is for Blizzard Sorceress
*
* Instructions:	See /d2bs/kolbot/libs/config/Builds/README.txt
*
* Skill IDs:	See /d2bs/kolbot/sdk/skills.txt for a list of skill IDs.
*
* Stat IDs:
*
* 	Strength	= 0
* 	Energy		= 1
* 	Dexterity	= 2
* 	Vitality	= 3
*

Finished Char Build:

	Stats													Base Stats
	----------												----------
 	Strength: 45 (35 points used)								10
 	Energy: 35 (no points)										35
 	Dexterity: 25 (no points) (+35 from gear)					25
 	Vitality: 460 (includes points from quests)					10

	Skills				Levelreq			SkillID			TotalPoints
	------------		--------			-------			-----------
	Ice Bolt			    1				  39				20 	- Done @ level 79
	Frozen Armor		    1				  40				 1 	- Done @ level 3
	Warmth 				    1				  37				 1 	- Done @ level 4
	Frost Nova			    6				  44				 1 	- Done @ level 7
	Ice Blast			    6				  45				20 	- Done @ level 8
	Static Field		    6				  42				 ? 	- Done @ level 6 *****PUMP SKILL QUEST POINTS HERE***** (12 + 4 = 16)
	Telekinesis			    6				  43				 1 	- Done @ level 6
	Shiver Armor		   12				  50				 0 	- Done @ level 12
	Teleport	 		   18				  54				 1 	- Done @ level 18
	Glacial Spike		   18				  55				20 	- Done @ level 93
	Blizzard	 		   24				  59				20 	- Done @ level 24
	Frozen Orb 			   30				  64				 0	- Done @ level 49
	Cold Mastery 		   30				  65				 ?	- Done @ level 64

	TOTAL Points Spent --------------------------------------> 106

	Attack Config Variables For Sorceress
	---------------------------------------------------------------------------------------------------------------------
	Config.AttackSkill[0] = -1; // Preattack skill.
	Config.AttackSkill[1] = 59; // Primary skill to bosses.
	Config.AttackSkill[2] = 45; // Primary untimed skill to bosses. Keep at -1 if Config.AttackSkill[1] is untimed skill.
	Config.AttackSkill[3] = 59; // Primary skill to others.
	Config.AttackSkill[4] = 45; // Primary untimed skill to others. Keep at -1 if Config.AttackSkill[3] is untimed skill.
	Config.AttackSkill[5] = -1; // Secondary skill if monster is immune to primary.
	Config.AttackSkill[6] = -1; // Secondary untimed skill if monster is immune to primary untimed.
*/
js_strict(true);

if (!isIncluded("common/Cubing.js")) { include("common/Cubing.js"); };
if (!isIncluded("common/Prototypes.js")) { include("common/Prototypes.js"); };
if (!isIncluded("common/Runewords.js")) { include("common/Runewords.js"); };

var AutoBuildTemplate = {

	1:	{	
			Update: function () {
				Config.AttackSkill = [-1, 36, -1, 0, -1, -1, -1];
				Config.LowManaSkill = [0,0];
				Config.HPBuffer = 2;
				Config.MPBuffer = 8;
				Config.RejuvBuffer = 4; 
			}
		},
		
	2:	{
			SkillPoints: [39], 											// Ice Bolt + 1 (level 1)
			StatPoints: [0, 0, 0, 0, 0],								// Strength 15
			Update: function () {
				Config.AttackSkill = [-1, 39, -1, 0, -1, 0, 0];
				Config.BeltColumn = ["hp", "hp", "mp", "mp"];
			}
		},
		
	3:	{
			SkillPoints: [40], 											// Frozen Armor + 1 (level 1)
			StatPoints: [3, 3, 3, 3, 3],								// Vitality 15
			Update: function () {
				Config.AttackSkill = [-1, 39, -1, 39, -1, 0, 0];
			}
		},
		
	4:	{
			SkillPoints: [37],											// Warmth + 1 (level 1)
			StatPoints: [0, 0, 0, 0, 0],								// Strength 20
			Update: function () {
				
			}
		},

	5:	{
			SkillPoints: [39],											// Ice Bolt + 1 (level 2)
			StatPoints: [3, 3, 3, 3, 1],								// Vitality 20
			Update: function () {

			}
		},

	6:	{
			SkillPoints: [45],											// Ice Blast + 1 (level 1)
			StatPoints: [0, 0, 0, 0, 0],								// Strength 25
			Update: function () {
				Config.AttackSkill = [-1,45,-1,45,-1,-1,-1];
			}
		},

	7:	{
			SkillPoints: [45, 42],										// Ice Blast + 1 (level 2), Static Field + 1 (level 1)
			StatPoints: [3, 3, 3, 0, 0],								// Strength 27, Vitality 23
			Update: function () {
				
			}
		},

	8:	{
			SkillPoints: [45],											// Ice Blast + 1 (level 3)
			StatPoints: [3, 3, 3, 1, 1],								// Vitality 28
			Update: function () {
				
			}
		},

	9:	{
			SkillPoints: [45],											// Ice Blast + 1 (level 4)
			StatPoints: [3, 3, 3, 1, 3],								// Vitality 33
			Update: function () {
				
			}
		},

	10:	{
			SkillPoints: [45],											// Ice Blast + 1 (level 5)
			StatPoints: [3, 3, 3, 3, 1],								// Vitality 38
			Update: function () {
			}
		},

	11:	{	
			SkillPoints: [45],											// Ice Blast + 1 (level 6)
			StatPoints: [3, 3, 1, 3, 3],								// Vitality 43
			Update: function () {
				
			}
		},

	12:	{
			SkillPoints: [50],											// Shiver Armor + 1 (level 1)
			StatPoints: [0, 0, 0, 0, 0],								// Strength 32
			Update: function () {
				Config.TownCheck = true;
				Config.LowManaSkill = [-1, -1];
			}
		},

	13:	{
			SkillPoints: [45],											// Ice Blast + 1 (level 7)
			StatPoints: [0, 0, 0, 0, 0],								// Strength 37
			Update: function () {
				
			}
		},

	14:	{
			SkillPoints: [45],											// Ice Blast + 1 (level 8)
			StatPoints: [3, 3, 3, 3, 3],								// Vitality 48
			Update: function () {
				Config.CastStatic = 10; 
			}
		},

	15:	{
			SkillPoints: [45],											// Ice Blast + 1 (level 9)
			StatPoints: [3, 3, 3, 3, 1],								// Vitality 53
			Update: function () {
				Config.PickitFiles.push("auto/MercWeapon.nip");
			}
		},

	16:	{
			SkillPoints: [45],											// Ice Blast + 1 (level 10)
			StatPoints: [0, 0, 0, 0, 0],								// Strength 42
			Update: function () {
				
			}
		},

	17:	{
			SkillPoints: [43],											// Telekinesis + 1 (level 1)
			StatPoints: [0, 0, 0, 0, 0],								// Strength 47
			Update: function () {
				
			}
		},

	18:	{
			SkillPoints: [54],											// Teleport + 1 (level 1) 
			StatPoints: [3, 3, 3, 1, 1],								// Vitality 58
			Update: function () {
				Config.OpenChests = false;			
				Config.Cubing = true;
				
			}
		},

	19:	{
			SkillPoints: [45],											// 	Ice Blast + 1 (level 11)
			StatPoints: [3, 3, 3, 3, 1],								// Vitality 63
			Update: function () {
				
			}
		},

	20:	{
			SkillPoints: [45],											// Ice Blast + 1 (level 12)
			StatPoints: [3, 3, 3, 3, 1],								// Vitality 68
			Update: function () {
				
			}
		},

	21:	{	
			SkillPoints: [45],											// Ice Blast + 1 (level 13)
			StatPoints: [3, 3, 3, 3, 1],								// Vitality 73
			Update: function () {
				
			}
		},

	22:	{
			SkillPoints: [55],											// Glacial Spike + 1 (level 1)
			StatPoints: [0, 0, 0, 0, 0],								// Strength 52
			Update: function () {
				
			}
		},

	23:	{
			SkillPoints: [44],											// Frost Nova + 1 (level 1)
			StatPoints: [0, 0, 0, 0, 0],								// Strength 57
			Update: function () {
				
			}
		},

	24:	{
			SkillPoints: [59],											// Blizzard + 1 (level 1)
			StatPoints: [0, 0, 0, 0, 0],								// Strength 62
			Update: function () {
				Config.BeltColumn = ["hp", "mp", "mp", "mp"];
				Config.AttackSkill = [-1, 59, 45, 59, 45, -1, -1];
				Config.Dodge = true;
				
			}
		},

	25:	{
			SkillPoints: [59,45],										// Blizzard + 1 (level 2), Ice Blast + 1 (level 14)
			StatPoints: [0, 0, 0, 0, 0],								// Strength 67
			Update: function () {
				
			}
		},

	26:	{
			SkillPoints: [59],											// Blizzard + 1 (level 3)
			StatPoints: [0, 0, 0, 3, 3],								// Strength 70 Vitality 75
			Update: function () {
				
			}
		},

	27:	{
			SkillPoints: [59, 45],										// Blizzard + 1 (level 4), Ice Blast + 1 (level 15)
			StatPoints: [3, 3, 3, 3, 3, 3, 3, 3, 3, 3],					// Vitality 85
			Update: function () {
				
			}
		},

	28:	{
			SkillPoints: [59],											// Blizzard + 1 (level 5)
			StatPoints: [3, 3, 3, 3, 3],								// Vitality 90
			Update: function () {
				
			}
		},

	29:	{
			SkillPoints: [59],											// Blizzard + 1 (level 6)
			StatPoints: [3, 3, 3, 3, 3],								// Vitality 95
			Update: function () {
				
			}
		},

	30:	{
			SkillPoints: [59,65],										// Blizzard + 1 (level 7), Cold Mastery + 1 (level 1)
			StatPoints: [3, 3, 3, 3, 3],								// Vitality 100
			Update: function () {
				
			}
		},

	31:	{	
			SkillPoints: [59, 65],										// Blizzard + 1 (level 8), Cold Mastery + 1 (level 2)
			StatPoints: [3, 3, 3, 3, 3],								// Vitality 105
			Update: function () {
				
			}
		},

	32:	{
			SkillPoints: [59],											// Blizzard + 1 (level 9)
			StatPoints: [3, 3, 3, 3, 3],								// Vitality 110
			Update: function () {
				
			}
		},

	33:	{
			SkillPoints: [59],											// Blizzard + 1 (level 10)
			StatPoints: [3, 3, 3, 3, 3],								// Vitality 115
			Update: function () {
				
			}
		},

	34:	{
			SkillPoints: [59],											// Blizzard + 1 (level 11)
			StatPoints: [0, 0, 0, 0, 0],								// Strength 75
			Update: function () {
				
			}
		},

	35:	{
			SkillPoints: [59],											// Blizzard + 1 (level 12)
			StatPoints: [0, 0, 0, 0, 0],								// Strength 80
			Update: function () {
				Config.LowGold = 30000;
			}
		},

	36:	{
			SkillPoints: [59],											// Blizzard + 1 (level 13)
			StatPoints: [0, 0, 0, 0, 0],								// Strength 85
			Update: function () {
				
			}
		},

	37:	{
			SkillPoints: [59],											// Blizzard + 1 (level 14)
			StatPoints: [0, 0, 0, 0, 0],								// Strength 90
			Update: function () {
				Config.Dodge = true;
			}
		},

	38:	{
			SkillPoints: [59],											// Blizzard + 1 (level 15)
			StatPoints: [0, 0, 0, 0, 0],								// Strength 95
			Update: function () {
				
			}
		},

	39:	{
			SkillPoints: [59],											// Blizzard + 1 (level 16)
			StatPoints: [3, 3, 3, 3, 3],								// Vitality 120
			Update: function () {
				
			}
		},

	40:	{
			SkillPoints: [59],											// Blizzard + 1 (level 17)
			StatPoints: [3, 3, 3, 3, 3],								// Vitality 125
			Update: function () {
				Config.LowGold = 35000;
			}
		},

	41:	{	
			SkillPoints: [59],											// Blizzard + 1 (level 18)
			StatPoints: [3, 3, 3, 3, 3],								// Vitality 130
			Update: function () {
				
			}
		},

	42:	{
			SkillPoints: [59],											// Blizzard + 1 (level 19)
			StatPoints: [3, 3, 3, 3, 3],								// Vitality 140
			Update: function () {
				
			}
		},

	43:	{
			SkillPoints: [59],											// Blizzard + 1 (level 20)
			StatPoints: [3, 3, 3, 3, 3,],								// Vitality 145
			Update: function () {
				
			}
		},

	44:	{
			SkillPoints: [65],											// Cold Mastery + 1 (level 3)
			StatPoints: [3, 3, 3, 3, 3],								// Vitality + 5
			Update: function () {
				
			}
		},

	45:	{
			SkillPoints: [65],											// Cold Mastery + 1 (level 4)
			StatPoints: [3, 3, 3, 3, 3],								// Vitality + 5
			Update: function () {
				Config.CastStatic = 35;
				Config.StaticList = ["Griswold", /*"Gargoyle Trap", */"Andariel", "Duriel", "Mephisto", "Izual"];
			}
		},

	46:	{
			SkillPoints: [65],											// Cold Mastery + 1 (level 5)
			StatPoints: [3, 3, 3, 3, 3],								// Vitality + 5
			Update: function () {
				
			}
		},

	47:	{
			SkillPoints: [65],											// Cold Mastery + 1 (level 6)
			StatPoints: [3, 3, 3, 3, 3],								// Vitality + 5
			Update: function () {
				
			}
		},

	48:	{
			SkillPoints: [65],											// Cold Mastery + 1 (level 7)
			StatPoints: [3, 3, 3, 3, 3],								// Vitality + 5
			Update: function () {
				
			}
		},

	49:	{
			SkillPoints: [65],											// Cold Mastery + 1 (level 8)
			StatPoints: [3, 3, 3, 3, 3],								// Vitality + 5
			Update: function () {
				
			}
		},

	50:	{
			SkillPoints: [65],											// Cold Mastery + 1 (level 9)
			StatPoints: [3, 3, 3, 3, 3],								// Vitality + 5
			Update: function () {
			
			}
		},

	51:	{	
			SkillPoints: [65],											// Cold Mastery + 1 (level 10)
			StatPoints: [3, 3, 3, 3, 3],								// Vitality + 5
			Update: function () {
				
			}
		},

	52:	{
			SkillPoints: [65],											// Cold Mastery + 1 (level 11)
			StatPoints: [3, 3, 3, 3, 3],								// Vitality + 5
			Update: function () {
				
			}
		},

	53:	{
			SkillPoints: [65],											// Cold Mastery + 1 (level 12)
			StatPoints: [3, 3, 3, 3, 3],								// Vitality + 5
			Update: function () {
				
			}
		},

	54:	{
			SkillPoints: [65],											// Cold Mastery + 1 (level 13)
			StatPoints: [3, 3, 3, 3, 3],								// Vitality + 5
			Update: function () {
				
			}
		},

	55:	{
			SkillPoints: [65],											// Cold Mastery + 1 (level 14)
			StatPoints: [3, 3, 3, 3, 3],								// Vitality + 5
			Update: function () {
				Config.LowGold = 50000;
			}
		},

	56:	{
			SkillPoints: [65],											// Cold Mastery + 1 (level 15)
			StatPoints: [3, 3, 3, 3, 3],								// Vitality + 5
			Update: function () {
				
			}
		},

	57:	{
			SkillPoints: [65],											// Cold Mastery + 1 (level 16)
			StatPoints: [3, 3, 3, 3, 3],								// Vitality + 5
			Update: function () {
				
			}
		},

	58:	{
			SkillPoints: [65],											// Cold Mastery + 1 (level 17)
			StatPoints: [3, 3, 3, 3, 3],								// Vitality + 5
			Update: function () {
				
			}
		},

	59:	{
			SkillPoints: [65],											// Cold Mastery + 1 (level 18)
			StatPoints: [3, 3, 3, 3, 3],								// Vitality + 5
			Update: function () {
				
			}
		},

	60:	{
			SkillPoints: [45],											// Ice Blast + 1 (level 16)
			StatPoints: [3, 3, 3, 3, 3],								// Vitality + 5
			Update: function () {
				
			}
		},

	61:	{	
			SkillPoints: [45],											// Ice Blast + 1 (level 17)
			StatPoints: [3, 3, 3, 3, 3],								// Vitality + 5
			Update: function () {
				
			}
		},

	62:	{
			SkillPoints: [45],											// Ice Blast + 1 (level 18)
			StatPoints: [3, 3, 3, 3, 3],								// Vitality + 5
			Update: function () {
				
			}
		},

	63:	{
			SkillPoints: [45],											// Ice Blast + 1 (level 19)
			StatPoints: [3, 3, 3, 3, 3],								// Vitality + 5
			Update: function () {
				
			}
		},

	64:	{
			SkillPoints: [45],											// Ice Blast + 1 (level 20)
			StatPoints: [3, 3, 3, 3, 3],								// Vitality + 5
			Update: function () {
				
			}
		},

	65:	{
			SkillPoints: [39], 											// Ice Bolt + 1 (level 11)
			StatPoints: [3, 3, 3, 3, 3],								// Vitality + 5
			Update: function () {
				Config.LowGold = 60000;
			}
		},

	66:	{
			SkillPoints: [39], 											// Ice Bolt + 1 (level 12)
			StatPoints: [3, 3, 3, 3, 3],								// Vitality + 5
			Update: function () {
				
			}
		},

	67:	{
			SkillPoints: [39], 											// Ice Bolt + 1 (level 13)
			StatPoints: [3, 3, 3, 3, 3],								// Vitality + 5
			Update: function () {
				
			}
		},

	68:	{
			SkillPoints: [39], 											// Ice Bolt + 1 (level 14)
			StatPoints: [3, 3, 3, 3, 3],								// Vitality + 5
			Update: function () {
				
			}
		},

	69:	{
			SkillPoints: [39], 											// Ice Bolt + 1 (level 15)
			StatPoints: [3, 3, 3, 3, 3],								// Vitality + 5
			Update: function () {
				
			}
		},

	70:	{
			SkillPoints: [39], 							// Ice Bolt + 1 (level 16) , Static Field + 4 (level 16) (Hell Den of Evil, Hell Radament, Hell Izual)
			StatPoints: [3, 3, 3, 3, 3],					// Vitality + 10 (Hell Lam Esen's Tome)
			Update: function () {
			}
		},

	71:	{	
			SkillPoints: [39], 											// Ice Bolt + 1 (level 17)
			StatPoints: [3, 3, 3, 3, 3],								// Vitality + 5
			Update: function () {
				
			}
		},

	72:	{
			SkillPoints: [39], 											// Ice Bolt + 1 (level 18)
			StatPoints: [3, 3, 3, 3, 3],								// Vitality + 5
			Update: function () {
				
			}
		},

	73:	{
			SkillPoints: [-1], 											// Ice Bolt + 1 (level 19)
			StatPoints: [3, 3, 3, 3, 3],								// Vitality + 5
			Update: function () {
				
			}
		},

	74:	{
			SkillPoints: [-1], 											// Ice Bolt + 1 (level 20)
			StatPoints: [3, 3, 3, 3, 3],								// Vitality + 5
			Update: function () {
				
			}
		},

	75:	{
			SkillPoints: [-1], 											// Ice Bolt + 1 (level 19)
			StatPoints: [3, 3, 3, 3, 3],								// Vitality + 5
			Update: function () {
				
			}
		},

	76:	{
			SkillPoints: [-1], 											// Ice Bolt + 1 (level 20)
			StatPoints: [3, 3, 3, 3, 3],								// Vitality + 5
			Update: function () {
				
			}
		},

	77:	{
			SkillPoints: [-1], 											// Ice Blast + 1 (level 2)
			StatPoints: [3, 3, 3, 3, 3],								// Vitality + 5
			Update: function () {
				
			}
		},

	78:	{
			SkillPoints: [-1], 											// Ice Blast + 1 (level 3)
			StatPoints: [3, 3, 3, 3, 3],								// Vitality + 5
			Update: function () {
				
			}
		},

	79:	{
			SkillPoints: [-1], 											// Ice Blast + 1 (level 4)
			StatPoints: [3, 3, 3, 3, 3],								// Vitality + 5
			Update: function () {
				
			}
		},

	80:	{
			SkillPoints: [-1],											// Ice Blast + 1 (level 5)
			StatPoints: [-1, -1, -1, -1, -1],								// Vitality + 5
			Update: function () {
				Config.Gamble = true;									// Time to spend dat ca$h!!
				Config.ScanShrines	= [];
			}
		},

	81:	{	
			SkillPoints: [45],											// Ice Blast + 1 (level 6)
			StatPoints: [-1, -1, -1, -1, -1],								// Vitality + 5
			Update: function () {
				
			}
		},

	82:	{
			SkillPoints: [45],											// Ice Blast + 1 (level 7)
			StatPoints: [-1, -1, -1, -1, -1],								// Vitality + 5
			Update: function () {
				
			}
		},

	83:	{
			SkillPoints: [45],											// Ice Blast + 1 (level 8)
			StatPoints: [-1, -1, -1, -1, -1],								// Vitality + 5
			Update: function () {
				
			}
		},

	84:	{
			SkillPoints: [45],											// Ice Blast + 1 (level 9)
			StatPoints: [-1, -1, -1, -1, -1],								// Vitality + 5
			Update: function () {
				
			}
		},

	85:	{
			SkillPoints: [45],											// Ice Blast + 1 (level 10)
			StatPoints: [-1, -1, -1, -1, -1],								// Vitality + 5
			Update: function () {
				
			}
		},

	86:	{
			SkillPoints: [45],											// Ice Blast + 1 (level 11)
			StatPoints: [-1, -1, -1, -1, -1],								// Vitality + 5
			Update: function () {
				
			}
		},

	87:	{
			SkillPoints: [45],											// Ice Blast + 1 (level 12)
			StatPoints: [-1, -1, -1, -1, -1],								// Vitality + 5
			Update: function () {
				
			}
		},

	88:	{
			SkillPoints: [45],											// Ice Blast + 1 (level 13)
			StatPoints: [-1, -1, -1, -1, -1],								// Vitality + 5
			Update: function () {
				
			}
		},

	89:	{
			SkillPoints: [45],											// Ice Blast + 1 (level 14)
			StatPoints: [-1, -1, -1, -1, -1],								// Vitality + 5
			Update: function () {
				
			}
		},

	90:	{
			SkillPoints: [45],											// Ice Blast + 1 (level 16)
			StatPoints: [-1, -1, -1, -1, -1],								// Vitality + 5
			Update: function () {
				
			}
		},

	91:	{	
			SkillPoints: [45],											// Ice Blast + 1 (level 17)
			StatPoints: [-1, -1, -1, -1, -1],								// Vitality + 5
			Update: function () {
				
			}
		},

	92:	{
			SkillPoints: [45],											// Ice Blast + 1 (level 18)
			StatPoints: [-1, -1, -1, -1, -1],								// Vitality + 5
			Update: function () {
				
			}
		},

	93:	{
			SkillPoints: [45],											// Ice Blast + 1 (level 19)
			StatPoints: [-1, -1, -1, -1, -1],								// Vitality + 5
			Update: function () {
				
			}
		},

	94:	{
			SkillPoints: [45],											// Ice Blast + 1 (level 20)
			StatPoints: [-1, -1, -1, -1, -1],								// Vitality + 5
			Update: function () {
				
			}
		},

	95:	{
			SkillPoints: [-1],											//
			StatPoints: [-1, -1, -1, -1, -1],								// Vitality + 5
			Update: function () {
				
			}
		},

	96:	{
			SkillPoints: [-1],											//
			StatPoints: [-1, -1, -1, -1, -1],								// Vitality + 5
			Update: function () {
				
			}
		},

	97:	{
			SkillPoints: [-1],											//
			StatPoints: [-1, -1, -1, -1, -1],								// Vitality + 5
			Update: function () {
				
			}
		},

	98:	{
			SkillPoints: [-1],											//
			StatPoints: [-1, -1, -1, -1, -1],								// Vitality + 5
			Update: function () {
				
			}
		},

	99:	{
			SkillPoints: [-1],											//
			StatPoints: [-1, -1, -1, -1, -1],								// Vitality + 5
			Update: function () {
				
			}
		}
};